...
func AddVenue(venue models.Venue) error {
	...
	_, err = tx.Exec(`INSERT INTO venue_configuration (venue_id, has_dual_monitor, has_led_lights, has_wled_lights, tts_voice, has_smartboard, 
		smartboard_uuid, smartboard_button_number, has_autodarts, autodarts_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, venueID, venue.Config.HasDualMonitor,
		venue.Config.HasLEDLights, venue.Config.HasWLEDLights, venue.Config.TTSVoice, venue.Config.HasSmartboard, venue.Config.SmartboardUUID,
		venue.Config.SmartboardButtonNumber, venue.Config.HasAutodarts, venue.Config.AutodartsURL)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func UpdateVenue(venueID int, venue models.Venue) error {
	...
	_, err = tx.Exec(`UPDATE venue_configuration SET has_dual_monitor = ?, has_led_lights = ?, has_wled_lights = ?, 
		tts_voice = ?, has_smartboard = ?, smartboard_uuid = ?, smartboard_button_number = ?, has_autodarts = ?, autodarts_url = ? WHERE venue_id = ?`,
		venue.Config.HasDualMonitor, venue.Config.HasLEDLights, venue.Config.HasWLEDLights, venue.Config.TTSVoice,
		venue.Config.HasSmartboard, venue.Config.SmartboardUUID, venue.Config.SmartboardButtonNumber, venue.Config.HasAutodarts, venue.Config.AutodartsURL, venueID)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func GetVenues() ([]*models.Venue, error) {
	...
	rows, err = models.DB.Query(`
		SELECT venue_id, has_dual_monitor, has_led_lights, has_wled_lights, tts_voice, has_smartboard, smartboard_uuid, smartboard_button_number, has_autodarts, autodarts_url
	...
	for rows.Next() {
		...
		err := rows.Scan(&config.VenueID, &config.HasDualMonitor, &config.HasLEDLights, &config.HasWLEDLights, &config.TTSVoice,
			&config.HasSmartboard, &config.SmartboardUUID, &config.SmartboardButtonNumber, &config.HasAutodarts, &config.AutodartsURL)
		if err != nil {
			return nil, err
		}
...
func GetVenueConfiguration(id int) (*models.VenueConfig, error) {
	...
	err := models.DB.QueryRow(`
		...
		smartboard_button_number, has_autodarts, autodarts_url FROM venue_configuration WHERE venue_id = ?`,
